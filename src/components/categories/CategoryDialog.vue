  ...
  <q-dialog v-model="modelValue" @hide="emits('closeDialog')">
    <q-card style="min-width: 50%" class="bg-primary text-white">
      <q-card-section class="row items-center q-pb-none">
        <div class="text-h5">
          {{ props.category ? $t("editCategory") : $t("addCategory") }}
        </div>
        <q-space />
      ...
      <q-form @submit="onSubmit" @reset="onReset" class="q-gutter-md q-py-lg">
        ...
        <q-card-actions align="center" class="text-primary">
          ...
          <q-btn
            flat
            :label="props.category ? $t('edit') : $t('create')"
            type="submit"
            color="blue-1"
            size="lg"
          />
...
const onSubmit = () => {
  const categoryModel = createNewCategory(name.value);

  store.dispatch("common/setIsLoading", true);
  if (props.category?.id) {
    api
      .put(`/categories/${props.category.id}`, categoryModel)
      .then((response) => {
